function [LL, QvalCell] = calc_LL_RL3WM_all_pers(pars,stimvaluesCell,corrCell,responseCell,test_learnblocknum,test_stimvalues,test_subjresp,condVec,logflag,fixparams)
%CALC_LL_RL3WM calculates the log-likelihood of data given parameters for
%RL3WM model
%
% =========================== INPUT VARIABLES ============================
% PARS: 1x12 vector of parameters 
%    [alpha_e, alpha_c, alpha_t, neg_alpha, epsilon, lambda, ns3, ns6, pers, tau, beta_test, beta]
...
for itrial = 1:nTestTrials
    ...
    if (subjresp ~= -1)
        try
        p = 1/sum(exp( beta_test.*( QvalCell{iblock}(istim,:)-QvalCell{iblock}(istim,subjresp) - pers.*(Cval-Cval(subjresp)) ) ) );
        p = (1-epsilon).*p + epsilon*1/3;
        catch
            blah = 1+1;
        end
