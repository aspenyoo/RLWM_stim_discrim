function [LL, QvalCell] = calc_LL_RLWMi_dn_i_all_pers(pars,stimvaluesCell,corrCell,responseCell,test_learnblocknum,test_stimvalues,test_subjresp,condVec,logflag,fixparams)
...
for iblock = 1:nBlocks
    ...
    for itrial = 1:nTrials
        ...
        if (ichoice ~= -1)
            ...
            delta = corrVec(itrial)-(ns(nStim/3)*imult(icond).*WMvalMat(istim,ichoice) + (1-ns(nStim/3)*imult(icond)).*QvalMat(istim,ichoice));
            if (corrVec(itrial)==1)
