function LL = calc_LL_WM_pers(pars,stimvaluesCell,corrCell,responseCell,filler,logflag,fixparams)
...
for iblock = 1:nBlocks
    ...
    for itrial = 1:nTrials
        ...
        if (ichoice ~= -1)
            p = 1/sum(exp( beta.*( valueMat(istim,:)-valueMat(istim,ichoice) - pers.*(Cval-Cval(ichoice))) ) );
        ...
        if (ichoice ~= -1)
            ...
            % update
            valueMat(istim,ichoice) = corrVec(itrial);
