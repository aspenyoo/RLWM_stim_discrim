function [respCell, correctCell] = simulate_RL3WM_neg(pars,stimvaluesCell,corrrespCell,condVec,subjrespCell)
%SIMULATE_RL3WM_NEG simulates data given parameters and stimuli
% 
% ========================= INPUT VARIABLES ============================
% PARS: 1x9 vector of parameters 
%    [alpha, neg_alpha_e, neg_alpha_c, neg_alpha_t, epsilon, lambda, ns3, ns6, beta]
%       alpha: RL learning rate for positive feedback
%       neg_alpha_e: RL learning rate for exemplar condition for negative feedback
%       neg_alpha_c: RL learning rate for category condition for negative feedback
%       neg_alpha_t: RL learning rate for text condition for negative feedback
%       epsilon: lapse rate
%       lambda: WM decay rate
%       ns3: WM (vs RL) weight for set size 3 blocks
%       ns6: WM weight for set size 6 blocks
%       beta: softmax inverse temperature parameters
    ...
    for itrial = 1:nTrials
        ...
        if (ichoice ~= -1)
            ...
            if (r==1);
                ...
                QvalMat(istim,ichoice) = QvalMat(istim,ichoice) + neg_alpha(icond)*(r-QvalMat(istim,ichoice));
            end
